...
wget -q -O /usr/bin/bbr "$Myrepo"/bbr.sh"
chmod +x /usr/bin/bbr
bbr >/dev/null 2>&1
...
sleep 1
wget -q -O /usr/bin/auto-set "$Myrepo"/auto-set.sh" && chmod +x /usr/bin/auto-set 
wget -q -O /usr/bin/crtxray "$Myrepo"/crt.sh" && chmod +x /usr/bin/crtxray 
sleep 1
